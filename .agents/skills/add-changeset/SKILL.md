...
A changeset declares which packages are affected by a change, the semver bump type, and a user-facing summary. It lives as a markdown file in `.changeset/` and is consumed by the configured release workflow to generate package versions and changelogs. Check the actual workflow before claiming publication.
...
The checked-in `.github/workflows/changesets.yml` runs `changesets/action` with
only a `version` command. It opens or updates a version PR; it does **not** publish
packages or create GitHub releases when that PR merges. Verify the current
workflow before describing release automation, since this configuration can change.

Keep version and changelog edits generated through `changeset version`. When
publication is already authorized, complete the repository's package validation
and packing workflow, publish the intended dependency-ordered packages, and
verify registry versions and artifact integrities. Forward required npm browser
authentication URLs and wait for successful CLI/registry confirmation. A source
merge or successful login alone does not establish publication.

Use pnpm-packed artifacts for workspace packages so dependency ranges and
`publishConfig` exports are resolved. Never publish a raw package directory with
unresolved `workspace:` dependencies. Maintain the release PR's update record
with actual publication results and remaining consumer/deployment work.
