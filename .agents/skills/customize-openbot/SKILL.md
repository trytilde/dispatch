...
1. Read `openbot.config.ts`, then inspect only the selected `agents/`, `skills/`, `sandbox/`, or `providers/` area.
...
4. Put reusable runtime instructions in `skills/<name>/SKILL.md`. Put sandbox files under `sandbox/assets/`; make `sandbox/bootstrap.sh` idempotent.
