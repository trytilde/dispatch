...
| Mobile, iOS | Xcode **16.1 or newer** + command line tools, an iOS 15.1+ simulator runtime, CocoaPods. Local builds are currently blocked on Xcode 26.5; use EAS for iOS artifacts, see `.agents/skills/run-expo/SKILL.md` | macOS only. React Native 0.86 enforces the Xcode minimum inside `pod install`, which fails with `Please upgrade XCode`; `mobile doctor` reads that minimum from the installed React Native and checks it up front |
