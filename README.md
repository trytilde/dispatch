...
The default inference provider is Vercel AI Gateway. Init asks for a human-readable API-key name, creates the key with the configured Vercel account, and stores it in SOPS as `AI_GATEWAY_API_KEY`. Newly scaffolded agents use `openai/gpt-5.6-sol` with medium reasoning through a direct `@ai-sdk/openai` import; the inference provider does not supply a model factory.
