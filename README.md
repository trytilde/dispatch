...
`openbot dev` reconciles Tilde resources for every authored agent before starting services. It creates or updates the local-running Vercel AI SDK endpoint and creates one dynamic MCP server and skill registry per agent. Their IDs are maintained as `AGENT_<ID>_*` values in `configuration/.env`; one-time endpoint credentials remain in encrypted configuration. Run the command under the Tilde tunnel when ChatKit must reach the local agent routes.
