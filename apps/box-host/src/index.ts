...
if (!Number.isSafeInteger(port) || port < 1 || port > 65_535) throw new Error("OPENBOT_BOX_PORT must be a valid port");

const server = createServer(connectNodeAdapter({ routes: registerBoxService, requestPathPrefix: "/rpc" }));
