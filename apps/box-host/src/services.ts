...
function authorized(context: HandlerContext): void {
  ...
  if (!token || token.length < 32)
    throw new ConnectError("Box host capability is not configured", Code.Unavailable);
  if (!validCapability(context.requestHeader.get("authorization"), token))
    throw new ConnectError("Box capability required", Code.PermissionDenied);
}
...
export function registerBoxService(router: ConnectRouter): void {
  router.service(BoxService, {
    ...
    async exec(request, context) {
      ...
      try {
        const result = await execute(request.command, request.arguments, {
          ...
          timeout: request.timeoutSeconds ? request.timeoutSeconds * 1000 : 120_000,
    ...
    async writeFile(request, context) {
      authorized(context);
      return { bytesWritten: BigInt(await writeWorkspaceFile(request.path, request.content)) };
    },
    async screenshot(_request, context) {
      authorized(context);
      const result = await execute(
        "import",
        ["-display", process.env.DISPLAY ?? ":1", "-window", "root", "png:-"],
        { encoding: "buffer", maxBuffer: 24 * 1024 * 1024, signal: context.signal },
      );
    ...
    async listPorts(_request, context) {
      authorized(context);
      const ports = (process.env.OPENBOT_EXPOSED_PORTS ?? "6080,4101")
        .split(",")
        .map(Number)
        .filter((port) => Number.isSafeInteger(port) && port > 0 && port <= 65_535);
      return {
        ports: ports.map((port) => ({ port, protocol: "tcp", url: `http://127.0.0.1:${port}` })),
      };
    },
  });
}
