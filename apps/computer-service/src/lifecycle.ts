...
function childPath(base: string, path: string): string {
  if (!path || isAbsolute(path)) throw new ConnectError("Lifecycle paths must be non-empty and relative", Code.InvalidArgument);
  const target = resolve(base, path);
  const child = relative(base, target);
  if (child.startsWith("..") || isAbsolute(child)) throw new ConnectError("Lifecycle path escapes its root", Code.InvalidArgument);
  return target;
}

async function stored(): Promise<StoredLifecycle | undefined> {
  try {
    return JSON.parse(await readFile(resolve(currentRoot(), "manifest.json"), "utf8")) as StoredLifecycle;
  } catch (error) {
...
export async function applyLifecycleBundle(request: ApplyLifecycleBundleRequest): Promise<boolean> {
  ...
  if (request.digest !== lifecycleBundleDigest(request)) throw new ConnectError("Lifecycle bundle digest does not match its content", Code.InvalidArgument);
  ...
  try {
    for (const file of request.files) {
      ...
      if (file.sha256 && file.sha256 !== actual) throw new ConnectError(`Lifecycle file digest mismatch: ${file.path}`, Code.InvalidArgument);
...
export async function runLifecycle(phase: LifecyclePhase, expectedDigest: string, signal?: AbortSignal): Promise<Array<{ scriptId: string; exitCode: number; stdout: string; stderr: string }>> {
  if (phase === LifecyclePhase.UNSPECIFIED) throw new ConnectError("Lifecycle phase is required", Code.InvalidArgument);
  const manifest = await stored();
  if (!manifest) throw new ConnectError("No lifecycle bundle is installed", Code.FailedPrecondition);
  if (expectedDigest && expectedDigest !== manifest.digest) throw new ConnectError("Lifecycle bundle digest changed", Code.FailedPrecondition);
  ...
  for (const script of manifest.scripts.filter((candidate) => candidate.phases.includes(phase))) {
    ...
    try {
      ...
      results.push({ scriptId: script.id, exitCode: 0, stdout: result.stdout, stderr: result.stderr });
    } catch (error) {
...
export function lifecycleBundleDigest(request: Pick<ApplyLifecycleBundleRequest, "files" | "scripts">): string {
  const hash = createHash("sha256");
  for (const file of [...request.files].sort((left, right) => left.path.localeCompare(right.path))) {
    hash.update("file\0").update(file.path).update("\0").update(String(file.mode)).update("\0").update(file.content).update("\0");
  }
  for (const script of [...request.scripts].sort((left, right) => left.id.localeCompare(right.id))) {
    hash.update("script\0").update(script.id).update("\0").update(script.path).update("\0");
    for (const phase of [...script.phases].sort((left, right) => left - right)) hash.update(String(phase)).update(",");
    hash.update("\0");
  }
