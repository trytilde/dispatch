...
describe("owner authentication", () => {
  ...
  it("completes browser PKCE login and establishes host-only token cookies", async () => {
    ...
    const callback = await app.request(
      `https://dispatch.test/auth/callback?code=code-one&state=${encodeURIComponent(authorizationInput?.state ?? "")}`,
    ...
    expect(established).toEqual(
      expect.arrayContaining([
        expect.stringContaining("dispatch_access=fresh-token"),
        expect.stringContaining("dispatch_refresh=refresh-one"),
      ]),
    );
  ...
  it("keeps the development callback on the browser origin", async () => {
    vi.stubEnv("PUBLIC_ORIGIN", "https://our-dispatch-control.vercel.app");
    const provider = stubProvider();
  ...
  it("uses the configured HTTPS origin for a matching remote development host", async () => {
    vi.stubEnv("PUBLIC_ORIGIN", "https://our-dispatch.exe.xyz");
    const provider = stubProvider();
    const app = createApp({ authProvider: provider, devMode: true, webRoot: "/missing" });

    const login = await app.request("http://127.0.0.1:4100/auth/login", {
      headers: {
        "x-forwarded-host": "our-dispatch.exe.xyz",
        "x-forwarded-proto": "http",
      },
    });
    ...
    expect(login.status).toBe(302);
    expect(provider.authorizationUrl).toHaveBeenCalledWith(
      expect.objectContaining({ redirectUri: "https://our-dispatch.exe.xyz/auth/callback" }),
    );
  ...
  it("refreshes an expired browser session and rotates the access cookie", async () => {
    ...
    provider.verify.mockImplementation(async (token) => {
      ...
      return { subject: "human-one", groups: [], scope: ["dispatch:control"] };
    });
    const app = createApp({ authProvider: provider, webRoot: "/missing" });
    const response = await app.request("/auth/session", {
      headers: { cookie: "dispatch_access=expired; dispatch_refresh=refresh-one" },
    });
    expect(response.status).toBe(200);
    expect(provider.refresh).toHaveBeenCalledWith("refresh-one");
    expect(response.headers.get("set-cookie")).toContain("dispatch_access=fresh-token");
  });
  ...
  it("requires a matching origin for unsafe cookie-authenticated requests", async () => {
    const app = createApp({ authProvider: stubProvider(), webRoot: "/missing" });
    const rejected = await app.request("https://dispatch.test/api/computer/missing/preview", {
      method: "POST",
      headers: { cookie: "dispatch_access=valid-token" },
    });
    expect(rejected.status).toBe(403);

    const accepted = await app.request("https://dispatch.test/api/computer/missing/preview", {
      method: "POST",
      headers: {
        cookie: "dispatch_access=valid-token",
        origin: "https://dispatch.test",
      },
    });
    expect(accepted.status).toBe(404);

    const bearer = await app.request("https://dispatch.test/api/computer/missing/preview", {
      method: "POST",
      headers: { authorization: "Bearer valid-token" },
    });
  ...
  it("accepts the configured matching HTTPS origin for remote development mutations", async () => {
    const app = createApp({
      authProvider: stubProvider(),
      devMode: true,
      environment: { PUBLIC_ORIGIN: "https://our-dispatch.exe.xyz" },
      webRoot: "/missing",
    });
    const headers = {
      cookie: "dispatch_access=valid-token",
      origin: "https://our-dispatch.exe.xyz",
      "x-forwarded-host": "our-dispatch.exe.xyz",
      "x-forwarded-proto": "http",
    };
...
function stubProvider() {
  return {
    ...
    verify: vi.fn(async () => ({ subject: "human-one", groups: [], scope: ["dispatch:control"] })),
