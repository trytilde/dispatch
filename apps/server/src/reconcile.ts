...
export async function reconcileRepository(
  input: { origin?: string; prune?: boolean } = {},
  ...
  return withLifecycleLease("repository-reconcile", async () => {
    ...
    if (!origin)
      return {
        ...
        skipped: "Public origin is not configured",
        skills: [],
        agents: [],
        errors: [],
      };
    if (!(await tildeEnvironment()))
      return {
        digest: repository.digest,
        skipped: "Tilde is not configured",
        skills: [],
        agents: [],
        errors: [],
      };
    ...
    return {
      digest: repository.digest,
      registryId: skills.registryId,
      skills: skills.items,
      agents,
      errors: agents
        .filter((agent) => agent.status === "error")
        .map((agent) => `Agent ${agent.id} failed to reconcile`),
    };
  });
}
...
async function reconcileSkills(repository: Awaited<ReturnType<typeof loadRepository>>) {
  ...
  const existingByName = Object.fromEntries(
    existing.filter((item) => item.remoteId).map((item) => [item.name, item.remoteId!]),
  ) as Record<string, string>;
  const registryId =
    existing.find((item) => item.registryId)?.registryId ??
    (await getEnvironment(environmentNames.tildeSkillRegistryId));
  const provider = await configuredProvider<SkillProvider>("skill");
  if (!provider.reconcileRegistry)
    throw new Error(`Skill provider ${provider.descriptor.id} does not implement reconciliation`);
  const result = await provider.reconcileRegistry(
    {
      name: repository.config.skills.registryName,
      description:
        repository.config.skills.registryDescription ?? "Skills committed with this OpenBot fork.",
  ...
  for (const remote of result.skills) {
    ...
    await db
      ...
      .onConflictDoUpdate({
        target: skillRegistrations.name,
        set: {
          remoteId: remote.id,
          registryId: result.id,
          digest: local.digest,
  ...
  for (const stale of existing.filter((item) => !desired.has(item.name))) {
    await db
      ...
      .where(eq(skillRegistrations.name, stale.name));
  }
  ...
  return {
    ...
    items: [
      ...repository.skills.map((skill) => ({
        name: skill.name,
        status: "ready",
        remoteId: result.skills.find((item) => item.name === skill.name)?.id,
      })),
      ...existing
...
async function reconcileAgents(
  ...
  for (const agent of repository.agents) {
    ...
    const endpointUrl = new URL(
      `${repository.config.agents.routePrefix}/${agent.id}`,
      origin,
    ).toString();
    ...
    let remoteId =
      prior?.remoteId ??
      (agent.id === "openbot" && provider.descriptor.id === "tilde-agents"
        ? tilde?.agentId
        : undefined);
    let status = "ready";
    try {
      if (!remoteId) {
        ...
        if (!registered.credentials)
          throw new Error(
            `Agent provider ${provider.descriptor.id} did not return initial credentials`,
          );
        ...
        await provider.update(
          remoteId,
          { displayName: agent.displayName, endpointUrl: new URL(endpointUrl) },
          providerContext(),
        );
      } else {
      ...
      await db
        ...
        .onConflictDoUpdate({
          target: agentRegistrations.sourceId,
          set: {
            providerId: provider.descriptor.id,
            remoteId,
            sourceDigest,
  ...
  for (const stale of existing.filter((item) => !desiredIds.has(item.sourceId))) {
    ...
    await db
      ...
      .where(eq(agentRegistrations.sourceId, stale.sourceId));
    output.push({
      id: stale.sourceId,
      status,
      ...(stale.remoteId ? { remoteId: stale.remoteId } : {}),
      endpointUrl: stale.endpointUrl,
    });
  }
  return output;
}
