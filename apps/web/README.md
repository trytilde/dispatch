...
The development server proxies `/healthz`, `/api/chat`, `/api/computer`, and `/auth` to the local control service with forwarded-origin headers enabled, so the control service can keep browser-facing OAuth callbacks on the Vite origin.
