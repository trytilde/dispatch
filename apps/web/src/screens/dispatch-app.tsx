...
export function DispatchApp() {
  useEffect(() => {
    void dispatchRuntime.actions.initialize({ workspace: true });
  }, []);
  ...
  const conversation = useStore(dispatchRuntime.store, (state) => state.conversation);
  const agentSetup = useStore(dispatchRuntime.store, (state) => state.agentSetup);
  const chatSearch = useStore(dispatchRuntime.store, (state) => state.search);
  ...
  function selectSidebarChat(chatId: string): void {
    ...
    for (const agent of agents) {
      ...
      void dispatchRuntime.actions.selectSession(agent.id, session);
      return;
    }
  }
  ...
  useEffect(() => {
    if (!searchOpen || !search.trim()) {
      dispatchRuntime.actions.clearSearch();
      return;
    }
    const handle = window.setTimeout(() => void dispatchRuntime.actions.searchChatKit(search), 250);
  ...
  function selectAgent(agent: ChatAgent): void {
    ...
    setThreadRootId("");
    restoredSessionRef.current = "";
    void dispatchRuntime.actions.selectAgent(agent.id);
  }
  ...
  async function send(event: FormEvent): Promise<void> {
    ...
    try {
      if (outgoingFiles.length > 0)
        activeSessionId = await dispatchRuntime.actions.ensureSession(
      ...
      await dispatchRuntime.actions.sendMessage({
        text,
        attachmentIds,
      ...
      dispatchRuntime.actions.setError(errorMessage(reason));
    }
  }
  ...
  async function stop(): Promise<void> {
    if (!sessionId) return;
    try {
      await dispatchRuntime.actions.interrupt();
    } catch (reason) {
      dispatchRuntime.actions.setError(errorMessage(reason));
    }
  }
  ...
  async function loadOlderMessages(): Promise<void> {
    ...
    await dispatchRuntime.actions.loadOlderMessages();
  }

  async function loadMoreAgents(): Promise<void> {
    if (!nextAgentToken) return;
    try {
      await dispatchRuntime.actions.loadMoreAgents();
    } catch (reason) {
      dispatchRuntime.actions.setError(errorMessage(reason));
    }
  }
  ...
  async function editQueuedTurn(turn: QueuedTurn): Promise<void> {
    ...
    await mutateQueue(() => dispatchRuntime.actions.removeQueuedTurn(turn.id));
  ...
  async function submitCreateAgent(candidateName: string, avatarId: string): Promise<void> {
    ...
    await dispatchRuntime.actions.startAgentSetup(name, avatarId);
  }

  const connectorActions: ConnectorPartActions = {
    busy: Boolean(connectorSetup?.submitting),
    onSelectAccount: (selection, account) => {
      void dispatchRuntime.client
        .bindConnector(agentId, account.id)
        .catch((reason) => dispatchRuntime.actions.setError(errorMessage(reason)));
    },
  ...
  async function submitConnectorSetup(input: ConnectorSetupSubmit): Promise<void> {
    ...
    try {
      ...
      if (result.status === "authorize" && result.authorization_url) {
        ...
        void waitForConnectorAccountActive(dispatchRuntime.client, {
          providerTypeId: selection.providerTypeId,
  ...
  async function finishConnectorSetup(result: CreateConnectorAccountResult): Promise<void> {
    await dispatchRuntime.client.bindConnector(agentId, result.account.id);
    closeConnectorSetup();
  ...
  return (
    ...
    >
      <WorkspaceSidebar
        ...
        onSignOut={() => void dispatchRuntime.actions.signOut()}
              ...
              <div className="message-list">
                ...
                {(() => {
                  ...
                  const rewriteUrl = (value: string) =>
                    dispatchRuntime.client.rewriteTildeUrl(value);
          ...
          <>
            <ActivityQueue
              ...
              onReorder={(id, queuePosition) =>
                void mutateQueue(() => dispatchRuntime.actions.reorderQueuedTurn(id, queuePosition))
              }
              onRemove={(id) =>
                void mutateQueue(() => dispatchRuntime.actions.removeQueuedTurn(id))
              }
              onRunNow={(id) => void mutateQueue(() => dispatchRuntime.actions.steerQueuedTurn(id))}
            />
            {composer}
          </>
        ) : null}
                ...
                <ConversationMessage role={threadRoot.role} createdAt={threadRoot.created_at}>
                  <MessageContent
                    ...
                    resolveAttachmentUrl={(selectedSessionId, attachmentId) =>
                      dispatchRuntime.client.getAttachmentDownloadUrl(
                        selectedSessionId,
                        attachmentId,
                      )
                    }
                    rewriteUrl={(value) => dispatchRuntime.client.rewriteTildeUrl(value)}
                  />
                </ConversationMessage>
              </div>
            ) : null}
      ...
      <AgentSetupDialog
        ...
        onClose={() => dispatchRuntime.actions.dismissAgentSetup()}
...
const SCROLL_STORAGE_KEY = "dispatch:chat-scroll";
