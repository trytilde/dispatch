...
import type { Agent, ChatMessage as OpenBotMessage, ChatSession, InstallationStatus, Sandbox } from "@openbot/contracts";
...
export function OpenBotApp() {
  ...
  if (gate === "loading") return <Centered><Brand /><p className="muted">Opening your workspace…</p></Centered>;
  if (gate === "locked") {
...
function Unlock({ onUnlocked, notice, setNotice }: { onUnlocked(): Promise<void>; notice: string; setNotice(value: string): void }) {
  ...
  return (
    <Centered>
      <div className="auth-card">
        <Brand />
        <div><p className="eyebrow">Private setup</p><h1>Unlock OpenBot</h1><p className="muted">Enter the setup code chosen when this deployment was created.</p></div>
        <form onSubmit={submit} className="stack">
          <label className="field-label">Setup code<input autoFocus type="password" value={code} onChange={(e) => setCode(e.target.value)} minLength={20} required /></label>
          {notice ? <p className="error" role="alert">{notice}</p> : null}
          <button className="primary" disabled={busy || code.length < 20}>{busy ? "Checking…" : "Continue"}</button>
        </form>
      </div>
    </Centered>
  );
...
function Setup({ status, onConfigured }: { status: InstallationStatus; onConfigured(value: InstallationStatus): void }) {
    ...
    <SetupShell step={step === "tilde" ? 1 : 2}>
      {step === "tilde" ? (
        <div className="setup-copy">
          <p className="eyebrow">Connect Tilde</p><h1>Deploy the shared agent gateway.</h1>
          <p className="lede">Tilde delivers signed ChatKit turns and tool calls to this OpenBot deployment. The state file creates the resources and returns the IDs you will paste in next.</p>
          <ol className="instructions"><li>Open Tilde in a new tab.</li><li>Select the organization and team for OpenBot.</li><li>Apply the prepared state and keep its output available.</li></ol>
          <a className="tilde-button" href={deployUrl.toString()} target="_blank" rel="noreferrer"><span className="tilde-mark">~</span> Deploy with Tilde <Arrow /></a>
          <button className="primary" onClick={() => setStep("secrets")}>I finished the Tilde deploy</button>
        </div>
      ) : (
        <form className="setup-form" onSubmit={configure}>
          <div><p className="eyebrow">Verify connections</p><h1>Add the state outputs.</h1><p className="muted">OpenBot stores provider credentials in {status.environmentProvider}. They are never written to the control database or returned to this browser.</p></div>
          <div className="field-grid">
            {status.environmentProvider.includes("Vercel") && !status.environmentProviderConfigured ? <Field name="vercelApiToken" label="Vercel access token" secret /> : null}
          ...
          <div className="button-row"><button type="button" className="secondary" onClick={() => setStep("tilde")}>Back</button><button className="primary" disabled={busy}>{busy ? "Verifying…" : "Verify and continue"}</button></div>
        </form>
      )}
    </SetupShell>
  );
...
function Onboarding({ status, onChanged }: { status: InstallationStatus; onChanged(value: InstallationStatus): void }) {
  const initial = Math.max(0, onboarding.findIndex((item) => item.id === status.onboardingStep));
  ...
  useEffect(() => {
    void agentClient.listAgents({}).then((result) => setPrimaryAgent(result.agents[0])).catch(() => undefined);
  }, []);
  ...
  return (
    <main className="onboarding-shell">
      <header className="onboarding-header"><Brand /><span>{index + 1} of {onboarding.length}</span></header>
      <section className="onboarding-card">
        <div className="onboarding-copy"><p className="eyebrow">{item.eyebrow}</p><h1>{item.title}</h1><p className="lede">{item.copy}</p>
          {item.id === "create" ? <div className="stack agent-form"><Field label="Agent name" name="agentName" value={name} onChange={setName} /><p className="muted">This renames the Tilde agent endpoint deployed during setup. Its browser, terminal, and files are provided by this OpenBot instance.</p><div className="tool-list"><span>Browser</span><span>Terminal</span><span>Files</span></div></div> : null}
        </div>
        <div className="onboarding-visual">{item.visual}</div>
      </section>
      <footer className="onboarding-footer"><div className="progress-dots">{onboarding.map((entry, i) => <button key={entry.id} aria-label={`Go to ${entry.eyebrow}`} className={i === index ? "active" : ""} onClick={() => void move(i)} />)}</div><div className="button-row"><button className="secondary" disabled={index === 0 || busy} onClick={() => void move(index - 1)}>Back</button><button className="primary" disabled={busy || (item.id === "create" && !name.trim())} onClick={() => void continueOnboarding()}>{last ? "Open workspace" : item.id === "create" ? "Save agent" : "Continue"}</button></div></footer>
    </main>
  );
}
...
function Workspace() {
  ...
  useEffect(() => {
    ...
    void chatClient.listSessions({ agentId: selectedAgentId }).then((result) => {
      setSessions(result.sessions);
      setSelectedSessionId((current) => result.sessions.some((session) => session.id === current) ? current : result.sessions[0]?.id || "");
      if (result.sessions.length === 0) setMessages([]);
    }).catch((error: unknown) => setChatError(error instanceof Error ? error.message : "Sessions could not be loaded."));
  }, [selectedAgentId]);

  useEffect(() => {
    if (!selectedSessionId) return;
    void chatClient.listMessages({ sessionId: selectedSessionId })
      .then((result) => setMessages(result.messages))
      .catch((error: unknown) => setChatError(error instanceof Error ? error.message : "Messages could not be loaded."));
  }, [selectedSessionId]);
  ...
  async function startComputer() {
    ...
    try {
      ...
      setDesktopUrl(desktop.url); setWorkspaceTab("Desktop");
...
function ChatMessage({ message }: { message: OpenBotMessage }) {
  return <article className={`message ${message.role}`}><div className="message-label">{message.role === "user" ? "You" : "OpenBot"}</div><p>{message.text}</p></article>;
}

function EmptyChat({ name }: { name: string }) { return <div className="empty-chat"><div className="openbot-glyph">✣</div><h1>What should {name} work on?</h1><p>Ask for research, a browser task, or work inside an isolated computer.</p><div className="suggestions"><button>Research a topic and summarize the evidence</button><button>Open a browser and inspect a website</button><button>Draft a plan, then execute its first step</button></div></div>; }
function DesktopEmpty({ onStart, busy }: { onStart(): void; busy: boolean }) { return <div className="desktop-empty"><div className="desktop-frame"><div className="browser-chrome"><i /><i /><i /></div><div className="desktop-wallpaper">✣</div></div><h3>No computer is running</h3><p>Start an isolated desktop for browser and computer work.</p><button className="primary" onClick={onStart} disabled={busy}>{busy ? "Starting computer…" : "Start computer"}</button></div>; }
function TerminalStub({ sandboxId }: { sandboxId: string }) { return <div className="terminal"><p><span>$</span> openbot sandbox status</p><p>{sandboxId ? `attached ${sandboxId}` : "no sandbox attached"}</p><i>Terminal streaming is the next control-plane connection.</i></div>; }
function FilesStub() { return <div className="pane-empty"><h3>Files</h3><p>Workspace files appear here after a computer starts.</p></div>; }
function ActivityStub() { return <div className="activity"><Thinking /><ApprovalCard /></div>; }
function SetupShell({ children, step }: { children: ReactNode; step: number }) { return <main className="setup-shell"><aside><Brand /><div className="setup-steps"><span className={step >= 1 ? "done" : ""}>1<i>Deploy Tilde</i></span><span className={step >= 2 ? "done" : ""}>2<i>Verify connections</i></span><span>3<i>Meet OpenBot</i></span></div><p>Your secrets stay encrypted at rest.</p></aside><section>{children}</section></main>; }
function Field({ label, name, secret, defaultValue, value, onChange }: { label: string; name: string; secret?: boolean; defaultValue?: string; value?: string; onChange?(value: string): void }) { return <label className="field-label">{label}<input required name={name} type={secret ? "password" : "text"} defaultValue={value === undefined ? defaultValue : undefined} value={value} onChange={onChange ? (e) => onChange(e.target.value) : undefined} autoComplete="off" /></label>; }
function Centered({ children }: { children: ReactNode }) { return <main className="centered">{children}</main>; }
function Brand() { return <div className="brand"><span>✣</span><strong>OpenBot</strong></div>; }
function Arrow() { return <svg viewBox="0 0 24 24" aria-hidden><path d="M5 12h14M13 6l6 6-6 6" /></svg>; }
function OrbitVisual() { return <div className="orbit"><span>✣</span><i /><i /><i /></div>; }
function ComputerVisual() { return <div className="computer-demo"><div className="browser-chrome"><i /><i /><i /></div><div className="demo-content"><span className="demo-sidebar" /><div><b /><b /><b /><em /></div></div><span className="cursor">↖</span></div>; }
function ReadyVisual() { return <div className="ready-visual"><span>✣</span><i className="pulse one" /><i className="pulse two" /><i className="pulse three" /></div>; }
