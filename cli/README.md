...
- `openbot new-agent` asks for an agent name, derives its kebab-case ID, and renders the fork-owned `configuration/templates/agent/**/*.hbs` tree under `configuration/agents/<id>/` without overwriting an existing agent. Agents use `openbot new-agent "Research Agent" --json` for a machine-readable result.
- `openbot dev` first reconciles each authored agent's Tilde Vercel AI SDK endpoint, dynamic MCP server, and skill registry, then generates contracts and supervises the combined control, agent, web, and optional Electron development processes. It persists non-secret `AGENT_<ID>_*` resource IDs in `configuration/.env` and newly issued endpoint credentials in SOPS. Use the Tilde tunnel when the local endpoint must receive ChatKit traffic.
