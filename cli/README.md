...
- `openbot tunnel -- <command>` runs a local service behind its Tilde local-runtime tunnel. `openbot plugin --cli <claude|codex|cursor|opencode|gemini>` configures selected Tilde MCP servers and skill registries; Codex, Claude Code, and Cursor also receive native hooks that record searchable ChatKit messages and canonical tool executions. Use `--agent-id` to select the audit agent, or the first visible agent is used. `--launch` optionally starts the configured harness.
