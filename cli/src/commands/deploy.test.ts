...
describe("deploy-prod", () => {
  it("parses the minimal deployment options", () => {
    expect(parseOptions(["--", "--yes", "--json"])).toEqual({ yes: true, dryRun: false, json: true, skipDeploy: false, service: "all" });
    expect(parseOptions(["--dry-run"])).toEqual({ yes: false, dryRun: true, json: false, skipDeploy: false, service: "all" });
    expect(parseOptions(["--skip-deploy", "--service", "agents"])).toEqual({ yes: false, dryRun: false, json: false, skipDeploy: true, service: "agents" });
