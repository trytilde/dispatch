import arg from "arg";
import { inkPrompts } from "./init-ui.js";
...
export async function runSecrets(argv: readonly string[]): Promise<void> {
  ...
  if (!operation || !name || extra.length) throw new Error("Usage: openbot secrets <set|unset> NAME");
  if (operation === "set") {
    if (!process.stdin.isTTY || !process.stdout.isTTY) throw new Error("openbot secrets set requires an interactive terminal");
