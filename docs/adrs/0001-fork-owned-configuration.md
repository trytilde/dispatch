...
Repository content is always discovered from canonical paths: agents from `configuration/agents/<id>/`, installation-level skills from `configuration/skills/`, custom provider source from `configuration/providers/`, sandbox assets from `configuration/sandbox/assets/`, and the sandbox bootstrap from `configuration/sandbox/bootstrap.sh`. These paths and the `/api/agents` route prefix are conventions, not `OpenBotConfiguration` options.
