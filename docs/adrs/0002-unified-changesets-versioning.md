...
- All Dispatch product packages one fixed group. Tilde SDK packages independent.
- Packages publish publicly. GitHub Action currently opens a version PR only.

## Context

Dispatch is a public-package monorepo whose application packages evolve as one product. Independent
...
Changesets manages release notes and package versions. Every Dispatch product package belongs to one
...
flowchart LR
  ...
  V --> G["Fixed Dispatch versions"]
  V --> S["Independent Tilde SDK versions"]
```

## Consequences

- One release number describes the complete Dispatch product package set.
...
- 2026-08-24T15:45:48+02:00: Kept the imported Tilde SDK packages outside Dispatch's fixed version group because their public API and external consumers form an independent compatibility boundary.
