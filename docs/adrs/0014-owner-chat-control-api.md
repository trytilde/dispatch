# ADR-0014: Owner chat through Tilde REST and direct ChatKit realtime

## In brief

- Web, desktop, and mobile preserve Tilde ChatKit's native REST, SSE, and realtime contracts.
...
flowchart LR
  ...
  O -->|"direct ChatKit realtime WebSocket"| T
```
...
- The client-owned ChatKit realtime dependency is checked against Tilde's generated AsyncAPI contract.
...
- 2026-08-21T12:00:00+01:00: Added the server-side ChatKit workspace WebSocket to background SSE adapter for team-wide agent activity, with the undocumented dependency isolated behind one allowlisted control-service boundary.
