# ADR-0021: OpenBot-owned UI naming and copy

## In brief

- OpenBot-authored UI owns its own names and its own words. No identifier or user-visible string carried from a reference build.
- One class prefix: `ob-`. Design tokens `--ob-*`, class families `ob-*`. No second prefix.
...
interface copy through verbatim into OpenBot-authored files. Nothing functional depended on either:
...
`pnpm test` cannot tell `ob-markdown__link` from `ui-markdown__link`, or one wording of an aria
...
Every OpenBot-authored surface carries OpenBot-authored names and OpenBot-authored wording.

Class families use the `ob-` prefix, matching the `--ob-*` custom properties already defined in
`packages/ui/src/openbot-ui.css`. One prefix, one owner. The single exception is CSS generic
...
is written for OpenBot rather than inherited. Account and identity defaults are generic
...
```mermaid
flowchart LR
  A["packages/ui/src/*\nOpenBot-authored"] -->|"ob-* classes,\nOpenBot copy"| P["Product surfaces"]
