...
- Follow-up (mobile OAuth return): pass `connectorAuthorizedReturnUrl(controlOrigin, "mobile")` from the native setup sheet, teach the landing page to redirect `client=mobile` to the app's link scheme (`openbot://` via `expo-linking`), handle the URL event in the app to close the sheet, and reuse `waitForConnectorAccountActive` to auto-send the hand-back message instead of the manual Done tap.
