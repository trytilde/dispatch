...
Built-ins use `openai`, `tilde-agents`, `tilde-chatkit`, `tilde-skills`, automatic local/Vercel sandbox selection, the environment provider, and GitHub publishing. Select a custom provider ID in configuration only after registering it under `providers/`.
