...
Forks can replace provider behavior without editing application imports. Create `providers/<plugin-id>/index.ts` and default-export `defineProviderPlugin(...)`. Register one or more typed factories with the builder, then select their IDs in `openbot.config.ts`.
