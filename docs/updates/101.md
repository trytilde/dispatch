...
Dispatch update together. The socket change is intentionally incompatible in
both directions, so mixed old/new API and Dispatch deployments are unsupported.
Run the API's additive per-user read-state migration before enabling the new
client. No Dispatch secret, environment, fork configuration, portable state, or
