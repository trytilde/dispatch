...
- `@trytilde/dispatch-control-service`: compare unsafe browser requests against the host-matched configured HTTPS development origin before localhost discovery or internal request-origin fallback.
...
- Production proof on `our-dispatch.exe.xyz`: matching public origin reaches the authentication boundary, foreign origin returns 403, and root/health remain 200.
