...
- Make `@trytilde/dispatch-ui` usable as a pnpm Git dependency from a clean consumer.
- Ensure public built subpaths such as `@trytilde/dispatch-ui/agent-avatar` exist before pnpm packs the selected workspace package.
...
```mermaid
flowchart LR
  C["pnpm Git consumer"] --> P["Dispatch workspace prepare"]
  P --> U["@trytilde/dispatch-ui prepare"]
...
- `@trytilde/dispatch-ui` runs its existing build from `prepare`, in addition to `prepack`.
...
- The Dispatch fixed package group receives a patch changeset.
- Repository checks and builds pass. Existing non-blocking lint warnings remain unrelated.

# Critical to apply to forks

yes — forks or applications that pin `@trytilde/dispatch-ui` directly from Git must update their immutable Dispatch commit to include this change. Keep `@trytilde/dispatch-workspace` and `@trytilde/dispatch-ui` allowed in pnpm build policy so the trusted Git dependency may generate and pack its artifacts. No state, secret, API, provider, or configuration migration is required.
