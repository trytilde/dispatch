...
The Tilde SDK now lives in the Dispatch packages monorepo under ADR-0030. This change exposes the
...
owner. Dispatch's public SDK packages translate authored local-tool activity into that API.

```mermaid
flowchart LR
  Agent["Authored Dispatch agent"] --> MCP["createMCPClient"]
...
- Documentation and release metadata
  ...
  - Adds a Changeset for all affected published packages and the fixed Dispatch package group.
  - Uses stable generated-source provenance instead of a machine-local filesystem path.

Validation completed:

- `pnpm tilde sdk refresh` passed: 632 operations validated; all five SDK packages built; 158
  focused tests passed.
- `pnpm tilde sdk validate` passed.
...
No client UI, protobuf, dependency, credential, secret, or persisted Dispatch configuration changes.
