...
- `.agents/skills/create-pr`: classifies configuration by the actual PR base repository. Canonical `trytilde/openbot` contributions keep only the ignore-all sentinel; fork PRs delete it and track initialized configuration while excluding `.env`.
...
- Update generation opened all 391 locally indexed coding-agent rollout files and analyzed 80,649 readable user/assistant messages. Thirty malformed JSONL lines could not be parsed, so those lines are an explicit evidence limitation; no raw or unrelated thread content was copied.
...
- On upstream contributions, remove every tracked fork configuration file except the canonical `configuration/.gitignore` sentinel. On fork PRs, commit initialized configuration and the sentinel deletion while keeping `.env` ignored.
