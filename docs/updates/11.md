...
- `@tryopenbot/configuration`: all provider roles are mandatory and `configuration/index.ts` is the sole composition root; agent bundles read their runtime environment directly.
...
- For `trytilde/openbot` contributions, track only canonical configuration sentinel. For fork PRs, track initialized configuration and exclude `.env`.
- Read ADR-0001 and ADR-0004 through ADR-0013 before conflict resolution.
