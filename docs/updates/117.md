...
```mermaid
flowchart LR
  CLI[tilde deploy] --> Git[Push named Code Storage branch]
...
- Changesets
  - Patch fixed Dispatch package group.

Validation: provider 19 tests pass; provider build passes; full `pnpm check` and `pnpm build` pass. Real our-dispatch deployment converged and public health, SPA, unsigned-agent rejection, and systemd service passed.
