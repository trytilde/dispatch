...
- `@trytilde/dispatch-client-runtime`: add participant identity/event contracts; hydrate and reconcile per-session participant activity; filter old `tilde.chatkit.participant` messages; add focused snapshot, realtime, and filtering tests.
- `@trytilde/dispatch-web`: merge participant activity with message chronology and render simple joined/left rows.
- `@trytilde/dispatch-ui`: add shared styling for quiet participant activity.
