...
- `@tryopenbot/client-runtime` validates supported proposal-tool output and
  submits the exact tokenless decision binding, then reloads durable status so
  a completed card cannot become pending again after navigation or restart.
...
- The browser and Electron renderer resume the conversation only after the
  ...
  provider details.
- The control service adds an owner-only decision proxy.
