...
Dispatch adds a propose-only agent tool, a stable SDK projection, an
...
flowchart LR
  ...
  Card -->|exact id + hash + generation| Proxy["Dispatch owner proxy"]
  Proxy --> Tilde
...
- `@trytilde/dispatch-client-runtime` validates supported proposal-tool output and
...
- `@trytilde/dispatch-ui` renders the proposal impact and Yes/No decision as a distinct
