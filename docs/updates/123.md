...
Give hosted Dispatch inference safe, exact billing without coupling billing to
...
Tilde remains the billing and authorization boundary. Dispatch stores no billing
...
- `@trytilde/dispatch-inference-provider`: writes
  `DISPATCH_HOSTED_INFERENCE_BILLING=1` only for managed Vercel project OIDC,
...
- `@trytilde/cli`: wires the controller into the generated agent webhook, returns a
...
- `@trytilde/dispatch-platform-integrations`: forwards the non-secret hosted billing
...
yes — rerun `tilde init` after merging so the selected inference provider
reconciles `DISPATCH_HOSTED_INFERENCE_BILLING` in fork-owned configuration.
