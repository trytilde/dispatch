...
durable AgentRun. The endpoint verifies the signed request's server-bound
synthesis session before any billing action, reserves managed Gateway credits,
...
flowchart LR
  ...
  P --> V[Verify bound synthesis session]
...
A completed redelivery is acknowledged without reopening inference. A prior
...
- `@trytilde/sdk-vercel-ai-node`: add
  ...
  synthesis-session verification, completion-result validation, and stable
...
- Tests: cover managed reservation/commit, BYOK release, insufficient credits,
  synthesis-session authorization, direct/Codex exclusion, failed completion,
...
- Documentation: amend the automatic-memory and durable-run ADRs, update the
  SDK public API contract, and add a Changeset for the published behavior.
...
- The materialized `memory-catcher/agent.ts` must use
  `createMemorySynthesisInferenceRun`, verify its server-bound synthesis
  session, attach all AI SDK model-call callbacks, and preserve the strict
