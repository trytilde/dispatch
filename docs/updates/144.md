...
After change, remote URL contains organization and repository only. Git gets repository JWT from process environment only while Git asks for credential. Checkout stores helper command, not bearer value. Helper is scoped to exact Code Storage host. It does not answer GitHub remote requests.
...
```mermaid
flowchart LR
  SOPS["SOPS / trusted service environment\nrepository JWT"] -->|"process environment"| Git["one Git fetch or push"]
...
- ephemeral runtime state: JWT copied into current Git child-process environment;
...
- `@tryopenbot/agent-service-provider`
  ...
  - authenticates initial clone/fetch transiently, then unsets the reconciliation copy;
