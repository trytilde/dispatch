...
- Reworks provider deployment into explicit build and phased reconciliation. Vercel and local control/agent artifacts receive only their required environment and secrets; generated deployment assets remain provider-owned.
...
- Narrows and consolidates provider contracts, including inference, chat, tools, skills, agent, Computer, runtime, platform integration, and service-provider boundaries. Concrete adapters remain outside core contract modules.
- Removes root `tilde.state.yaml`; provider-owned provisioning is tracked separately rather than pretending portable state is already implemented.
...
- `packages/chat-provider`: new chat contract and Tilde adapter for ChatKit sessions, history, attachments, and streaming.
...
- `packages/runtime-provider`: phased, idempotent provider build/reconcile/deploy coordination with durable checkpoint semantics.
- `packages/inference-provider` and `packages/platform-integrations`: consolidated inference boundary plus reusable Tilde/Vercel HTTP, registry, deployment, and AI Gateway adapters.
- `packages/skills-provider` and `packages/tools-provider`: narrowed contracts with Tilde discovery and reconciliation implementations.
- `packages/configuration`: explicit provider composition and instrumentation contract updates.
...
2. Update imports from removed or consolidated provider packages, especially the inference-provider and platform-integration boundaries. Keep provider contracts in each package's `src/core.ts` or `src/core/index.ts`.
...
7. Remove assumptions that root `tilde.state.yaml` or root `.vercelignore` owns deployment behavior. Provider lifecycles now own reconciliation and generated deploy assets.
8. Review custom Vercel/local providers against phased build and deployment contracts, environment allowlists, secret installation, and service-specific deploy behavior.
9. Review custom agents against the Eve-compatible directory subset and fixed-agent Computer tools. Existing deployed workspace seeds are one-time and are not overwritten by later repository edits.
10. Run `pnpm check`, `pnpm build`, `pnpm test:e2e`, and `pnpm --filter @openbot/desktop package`; verify login, refresh, logout, wrong-audience rejection, owner chat, attachments, Computer preview, and deployment dry-run on the customized fork.
