...
- Give OpenBot a second owner client without duplicating chat behavior a third time.
...
- `apps/web` — migrated onto the runtime. `src/chat-api.ts` deleted; roughly thirty component-level state variables in `screens/openbot-app.tsx` replaced by runtime state. New `src/runtime.ts` and `src/web-attachments.ts`.
...
- `packages/computer-service-provider` — renamed from `computer-provider`. Root provider API only; `./tools` export and the `@tryopenbot/computer-tools` dependency removed.
- `packages/auth-provider` — native PKCE configuration surfaced for the mobile flow.
