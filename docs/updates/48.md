...
- Move Expo, emulator, and toolchain logic out of untested `apps/mobile/scripts/*.mjs` into the published Tilde CLI, so fork developers and sandboxed agents get the same versioned tooling.
...
```mermaid
flowchart LR
  R["root scripts: dev:mobile:*, connect, dev:remote, doctor"] --> C["Tilde CLI"]
...
- One CLI owns operator commands and developer workflow. A separate `@trytilde/dispatch-dev-cli` package was built and folded back in before publication; ADR-0018's Updates record the reversal.
...
- `apps/mobile` — `scripts/*.mjs` deleted; package scripts delegate to `tilde`. No source change.
...
- `.changeset/fix-local-oauth-callbacks.md` — repaired: it arrived on `main` with PR 45 naming the pre-rename `@trytilde/dispatch-computer-provider`, which broke `pnpm changeset status` for everyone.
...
**Root script names changed.** Anything automating the old `pnpm --filter @trytilde/dispatch-mobile emulator` should call `pnpm dev:mobile:emulator` or `tilde mobile emulator`. CI that shells the mobile scripts directly needs no change.
...
**Check the changeset repair.** A fork carrying its own copy of `.changeset/fix-local-oauth-callbacks.md` from PR 45 has the same broken package name and the same `pnpm changeset status` failure; rename `@trytilde/dispatch-computer-provider` to `@trytilde/dispatch-computer-service-provider` there too.
