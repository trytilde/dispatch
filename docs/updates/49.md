...
- Init derives the fork repository (`GIT_GITHUB_REPOSITORY`) from the checkout's `origin` remote
  during provider initialization — the repository bootstrap already collected it, so init asks no
  new question. The provider is constructed in both configuration templates; `providers.git` is an
