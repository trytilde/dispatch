...
- Init derives the fork repository (`GIT_GITHUB_REPOSITORY`) from the checkout's `origin` remote
  and starts GitHub App provisioning, printing the authorization URL while the owner is still at
  the terminal (`git.github.authorization.required`); provisioning failures degrade to a
  `git.github.initialize.skipped` event and never fail init. The repository bootstrap already
  collected the fork slug, so init asks no new question. The provider is constructed in both
  configuration templates; `providers.git` is an optional composition slot, and the deployment
  lifecycle remains the idempotent finisher.
