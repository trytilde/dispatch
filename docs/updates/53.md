...
- Filtering stays in Dispatch, not cmdk. `commandProps={{ shouldFilter: false }}` — the agent list is already filtered by the caller through `value`/`onChange`, and only the *Actions* group filters locally against its own label and keyword strings. Two filters over one list would fight.
...
- `GlideMenu` (a Dispatch-authored atom from PR 50) owns the hover highlight for the whole list; rows carry `data-menu-row` and render their own selected fill. Hover and selection are separate layers on purpose.
...
**Theme switching is now a product surface.** The palette writes the `dispatch.theme` preference through `theme.ts`. A fork that shipped its own theme control writes the same key; reconcile before both are reachable.
