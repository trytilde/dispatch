...
- The step machine and its timing mirror the reference build's first-run structure; every user-visible string is OpenBot's own, per ADR-0021. Structure borrowed, wording not.
...
- **The gate stopped using the shared client runtime.** The previous `auth-gate.tsx` read `state.auth` from `openBotRuntime.store` and called `openBotRuntime.actions.initialize()`. It now calls `fetch("/auth/session")` directly and branches on `window.openbotDesktop` itself. `apps/web/src/runtime.ts` still builds a `ClientAuthAdapter` with exactly that web/desktop branch and still hands it to `createOpenBotRuntime`. Two auth paths now exist in one app and only one of them is used by the gate. This is in tension with ADR-0017, which makes the runtime mandatory for major UX surfaces and state interactions.
- **The onboarding result is a dead end.** `OnboardingResult` — name, colour, shape, selected tools — is JSON-stringified into `openbot.onboarding-result` and nothing at `HEAD` reads that key. No agent is created, no tool grant is issued. The tool picker records preferences that reach nothing.
...
- `apps/web` — `auth-gate.tsx` rewritten: local `Session` type, `loadSession` over `fetch("/auth/session")` with a `window.openbotDesktop.authStatus()` branch, `signIn` over `/auth/login` with a desktop branch, `seen` flag from `localStorage`, `Onboarding` rendered for both signed-out and not-yet-onboarded states. `openBotRuntime` import dropped from this file.
...
**A fork's users see a first-run flow they did not write.** Seven frames of OpenBot product copy — "OpenBot", "Stand up a bot, hand it the work you keep putting off.", "Every bot gets its own computer", "One bot, one job", "Which tools should your bots reach?", "Set up your bot" — render before a fork's app does, for every existing user, because the `openbot.onboarding-seen` key has never been set. A rebranded fork must pass its own copy or replace the component; there is no prop for the strings today, so replacing means forking `onboarding.tsx` or rendering something else from the gate.
...
**The gate no longer goes through the client runtime.** `auth-gate.tsx` calls `fetch("/auth/session")` and `window.openbotDesktop` directly. A fork that swapped in its own `ClientAuthAdapter` in `apps/web/src/runtime.ts` — a different session endpoint, a different sign-in redirect, a non-Electron desktop bridge — finds the gate ignoring it. Either reroute the gate through `openBotRuntime` or apply the same change twice. Check this before assuming a fork's auth still works.

**Two new `localStorage` keys are owned by this feature:** `openbot.onboarding-seen` and `openbot.onboarding-result`. A fork with its own first-run flag must reconcile, or users get both. Clearing site data replays onboarding.
...
**New published exports.** `Onboarding`, `OnboardingProps`, `OnboardingResult`, `OnboardingStep` from `@tryopenbot/ui`. Additive.
