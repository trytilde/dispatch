...
- Per-agent Mission Control channels; Tilde routes sessions via each channel's default agent. Only the primary agent gets the brokered GitHub toolkit (deployment context distinguishes primary from subagent).
...
- `packages/agent-provider` — per-agent Mission Control channels; brokered GitHub toolkit for the primary only; reconciliation tolerates a purged tool group; agent instructions distinguish direct tools from registry meta-tools.
