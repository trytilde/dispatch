...
- Take the software lifecycle away from agents. A background orchestrator (`dispatch orchestrate`, ADR-0019) owns build/verify/publish/deploy; agents stay lifecycle-unaware and only edit source. Every subagent can edit itself.
...
flowchart LR
  ...
  CS -->|"exec tilde new-agent"| Dev["Trusted dev sandbox\nfork checkout /workspace/dispatch"]
  Agent["Any agent (self-edit tools)"] --> Dev
  Orch["dispatch orchestrate\n(supervised in sandbox)"] -->|"watch edits"| Dev
  ...
  Dev -->|"git via reverse proxy"| GH[("GitHub fork\ndispatch/sandbox-edits")]
  GP["git-provider"] -->|"broker GitHub App credential,\nreconcile proxy profiles"| Tilde[("Tilde")]
```

- `GitHubGitProvider` brokers a GitHub App credential through Tilde (`server_token_exchange` via provider-app provisioning). Never a raw PAT. Reconciles `dispatch-github-rest` (api.github.com) and `dispatch-github-git` (github.com) reverse-proxy profiles; persists `GIT_GITHUB_*`.
...
- Orchestrator state machine: `deployed → live → publishing`. First edit flips every agent to the tunnel — whole-repo, never per-agent, because shared files affect any agent. After 30 s without file changes it verifies the project, pushes to `dispatch/sandbox-edits` through the git proxy, redeploys agent services, flips agents back to deployed endpoints. A failed stage leaves agents on the tunnel and retries after the next settle. Watcher ignores lifecycle-owned paths (`configuration/.env`, `secrets.enc.yaml`, `.sops.yaml`) — no self-trigger.
...
- `apps/control-service` — `POST /api/agents` (`agent-create.ts`) execs `tilde new-agent` in the dev sandbox.
...
**Primary agent ID changed: `hello-world` → `factory`. Breaking.** The fork keeps its existing `configuration/agent/` directory, but discovery now registers the primary as `factory`. Redeploy agents once (`pnpm tilde deploy --yes --service agents`). Secrets migrate: `AGENT_HELLO_WORLD_API_KEY` is dead; the next reconcile mints `AGENT_FACTORY_API_KEY` and registers the ChatKit endpoint with fresh credentials. Chat history on the old hello-world inbox does not carry over.
...
**Do not repurpose `dispatch/sandbox-edits`.** The orchestrator force-owns that branch for automated publishes. Merging it into the default branch stays an explicit owner (or requested-agent) pull request — no auto-merge yet.
