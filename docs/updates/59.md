...
- Give Dispatch a published mobile identity: Tilde publishes, Dispatch is the app. One EAS project, one identifier, upstream only, with the fork guard as code rather than a comment.
...
flowchart LR
  ...
  WF -->|"tilde check"| Gate["repository gates"]
  WF -->|"tilde mobile release build"| CLI["cli: release.ts"]
  ...
  Fork["a fork"] -->|"DISPATCH_EAS_PROJECT_ID"| Own["the fork's own EAS project"]
```

- ADR-0027 records store publication. `apps/mobile/app.json` becomes `app.config.ts`, so store identity resolves from the environment with the official values as defaults: `DISPATCH_EAS_PROJECT_ID`, `DISPATCH_APP_ID`, `DISPATCH_EXPO_OWNER`, plus name, slug, and scheme. The identifier is reverse-DNS of the publisher, `ai.trytilde.dispatch`, not of the product.
- `tilde mobile release build|submit|status|credentials|install` is the only publication path. The guard refuses when the resolved EAS project is the official one and `origin` is not `trytilde/dispatch`, naming the override a fork needs. `build` and `submit` require `--yes`; `install` and `status` do not, because neither spends minutes nor changes a listing.
...
- `apps/mobile` gains an `eas-build-post-install` script. EAS never invokes the Tilde CLI, so the CLI's unbuilt-dependency guard could not help there; the hook builds `client-runtime` inside the EAS worker instead.
...
**Bundle identifier changed: `dev.dispatch.mobile` → `ai.trytilde.dispatch`.** A fork that had installed its own build under the old identifier gets a separate app, not an upgrade. A fork publishing its own app should set `DISPATCH_APP_ID` in `configuration/.env` rather than inherit either value.
...
**Publication is upstream-only and does not need setup.** `tilde init` still never asks about EAS. A fork that wants its own app sets `DISPATCH_EAS_PROJECT_ID`, `DISPATCH_APP_ID`, and `DISPATCH_EXPO_OWNER`; without them, `tilde mobile release` refuses and names the overrides.

**New external requirements, all checked by `tilde mobile doctor`.** A JDK 17 or 21 — JDK 25 fails `:react-native-worklets:configureCMakeDebug` inside a restricted `java.lang.System` method. The React-Native-pinned NDK and CMake, which `tilde mobile setup` now installs. Xcode 16.1 or newer for iOS, with the minimum read from the installed React Native. Re-run `pnpm tilde mobile doctor` after pulling.
