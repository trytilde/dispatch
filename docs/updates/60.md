...
- The guard refuses when the resolved bucket is the official one and `origin` is not `trytilde/openbot`, reusing `isUpstreamRepository` from ADR-0027. A GitHub OIDC role scoped to `repo:trytilde/openbot:*`, added in identity-terraform, is the backstop rather than the only fence.
