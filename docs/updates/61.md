...
- Root workspace import `@tryopenbot/auth-provider` but never declare it. Declare it.
- Computer image test match literal `openbot-computer`, so any fork with other repository name fail. Match shape, not name.
- `@tryopenbot/ui` render with `react-dom`, so declare it peer.
...
flowchart TD
  ...
  RR -->|"explicit OPENBOT_REPOSITORY_ROOT"| EX["Use it (unchanged)"]
  RR -->|"walk up to @tryopenbot/workspace manifest"| WS["Workspace root"]
...
- Root discovery key on manifest name `@tryopenbot/workspace`. Explicit override still win first. No manifest anywhere up the chain still fall back to invocation directory, so `openbot init` in empty directory behave same.
...
- `openbot` (CLI)
  - `src/paths.ts`: add `findOpenBotWorkspaceRoot`, walk upward from invocation directory, read each `package.json`, stop at name `@tryopenbot/workspace`. Unreadable manifest keep walking.
...
- Workspace root
  - `package.json`: declare `@tryopenbot/auth-provider` dependency.
...
- Proof
  ...
  - `pnpm --filter openbot test`: 24 files, 117 tests passed.
  - `pnpm --filter @tryopenbot/computer-service-provider test`: 2 files, 19 tests passed.
...
Fork that run `openbot dev` through the workspace task runner hit the startup failure and the connection-refused race directly, and any fork whose repository is not named `openbot` hit the image test failure.
...
- Fork that rename the root package away from `@tryopenbot/workspace` keep old behavior: root resolve fall back to invocation directory. Rename back or set `OPENBOT_REPOSITORY_ROOT`.
