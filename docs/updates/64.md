...
- ADR-0018 remains the governing decision: one published Tilde CLI owns operator and developer
  workflows.
...
```mermaid
flowchart LR
  O["Operator or developer"] --> L["Interactive Dispatch launcher"]
...
surface should merge the update, install their normal locked dependencies, rebuild `@trytilde/cli`, and
run `pnpm --filter @trytilde/cli test`.
