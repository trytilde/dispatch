...
- Credentials never in chat. New-account forms post to owner-auth `/api/connectors/*`; control
  ...
  desktop and mobile bounce through `dispatch://` deep link.
...
- New runtime utility package `@trytilde/dispatch-connector-tools` for authored agents, sibling of
...
- `apps/desktop`
  - No code change needed: the existing `dispatch://` protocol handler already focuses the window;
...
`@trytilde/dispatch-connector-tools`), then a dev boot or deployment so the reconciler maps the
