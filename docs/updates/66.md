...
- Make Cua Driver the single programmatic GUI backend in every Dispatch Computer.
...
- Give every agent canonical Cua guidance plus a Dispatch-owned computer-use overlay, with an idempotent path from the bundled fallback to Tilde's managed skill.
...
- `@trytilde/dispatch-computer-tools` eagerly builds the AI SDK tool set, rejects name collisions, forwards cancellation, and uploads returned images through Tilde's session attachment boundary.
- Agent Provider owns registry reconciliation: the Dispatch overlay is permanent, the trusted managed Cua skill is preferred, and the provenance-recorded snapshot is removed only when the managed skill is available.
...
- `packages/agent-provider`
  - Reconcile the permanent Dispatch guidance, prefer the trusted managed Cua provider, and bundle a provenance-recorded canonical fallback with migration tests that preserve user-owned skills.
