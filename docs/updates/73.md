...
One open-source codebase. Keep Dispatch name. Fold `trytilde/harness-sdk` source into Dispatch.

- Remove cross-repo SDK release/pin dance.
- Rename public packages from Harness language to `@trytilde/sdk*`.
- Keep generated Tilde API client beside hand-authored wrappers and real Dispatch consumers.
- One `tilde` command surface from `@trytilde/cli`. No second CLI or plugin helper package.
- Shared JSON guards/accessors. No repeated `isRecord`, `isJsonObject`, `stringField` helpers at call sites.

# Architecture changes

Tilde API remains external contract authority. Dispatch repository now owns generated TypeScript
...
flowchart LR
  ...
  SDK --> OB["Dispatch providers and agents"]
  CLI["Tilde CLI"] --> CORE["auth, state, tunnel"]
  CLI --> PLUGIN["plugin setup"]
  CLI --> DEV["sdk refresh, validate, smoke, publish"]
  UTIL["@trytilde/dispatch-utilities/json"] --> OB
...
- `tilde auth|state|tunnel` owns core Tilde operations. `tilde plugin` owns coding-agent MCP and
  skill-registry setup. `tilde sdk` owns SDK lifecycle; publication requires `--yes`.
- SDK versions stay outside Dispatch fixed Changesets group. ADR-0002 amended. ADR-0030 added.
- Legacy auth and tunnel state remains readable. New writes use Harness-free paths.
- Public SDK never depends on `@trytilde/dispatch-*`. SDK JSON helpers live at `@trytilde/sdk/json`;
  Dispatch-only helpers live at `@trytilde/dispatch-utilities/json`.
...
- `@trytilde/cli`: imports all former Tilde CLI behavior; adds `auth`, `state`, `tunnel`, `plugin`, and
  `sdk` commands; keeps one interactive launcher/help inventory.
- `@trytilde/dispatch-agent-provider`, `git-provider`, `platform-integrations`, `computer-tools`: workspace
  SDK imports replace registry and Git SHA Harness dependencies.
- `@trytilde/dispatch-utilities`: domain-neutral JSON types, guards, string accessors, and non-throwing
  parser exposed through `@trytilde/dispatch-utilities/json`.
- `@trytilde/dispatch-connector-tools`, `computer-tools`, `computer-service`: repeated record guards removed;
...
- Replace standalone `tilde auth|state|tunnel` commands with `tilde auth|state|tunnel`.
- Replace coding-agent wrapper binaries or `@trytilde/harness-plugins` with `tilde plugin`.
