...
Make `tilde init` safe to resume after partial setup and explicit about independent runtime and inference choices.
...
```mermaid
flowchart LR
  I[tilde init] --> D[Detect current composition]
  D --> R[Runtime stage]
  D --> F[Inference stage]
...
- Tilde CLI: detects the current provider group, resumes incomplete setup, prompts per changed provider domain, writes checkout-local user configuration, and safely prepares inference template migration.
- `@trytilde/dispatch-inference-provider`: exposes stable provider-owned template contributions used for comparison and migration.
- `@trytilde/dispatch-configuration`: documents the checkout-local user configuration contract.
...
Fork operators must rerun `tilde init` after merging. If an older `~/.dispatch/config.json` supplied SOPS lookup metadata, choose or confirm that identity so Dispatch writes the new checkout-local `local-user-config.json`. Do not commit that file.
