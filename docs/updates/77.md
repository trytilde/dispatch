...
flowchart LR
  ...
  L --> R[Registration and ChatKit workspace]
  L --> K[Skills and registries]
  L --> W[Tools and assignments]
```

- CLI lifecycle code owns filesystem atomicity and cleanup.
- The agent provider owns external identity, registration, ChatKit workspace, skills, and tools.
...
- Tilde CLI: stages complete agent templates before lifecycle publication, preserves explicit rollback, and passes avatar/template inputs through one atomic creation path.
- `@trytilde/dispatch-agent-provider`: shares bounded fetch across clients, runs independent post-identity reconciliation concurrently, preserves PR 74's refreshed managed Cua identifiers, and adds race-safe authored skill adoption without overwriting user-owned collisions.
- `@trytilde/dispatch-platform-integrations`: accepts the bounded fetch implementation without changing its public connection authority.
