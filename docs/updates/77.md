...
- `@tryopenbot/agent-provider`: shares bounded fetch across clients, runs independent post-identity reconciliation concurrently, and repairs managed skill registry/tool convergence.
...
- Validation: 25 focused CLI and agent-provider tests passed. The complete reconciled branch passed build and browser checks. Repository-wide check on this isolated base remains blocked by two unrelated errors fixed in PR 75.
