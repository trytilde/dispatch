...
- Drive background activity, unread previews, and busy indicators from durable Mission Control events.
...
```mermaid
flowchart LR
  T[Tilde tools skills ChatKit Mission Control] --> C[control-service owner routes]
...
- `@tryopenbot/client-runtime`: adds plugin contracts, assignment client calls, durable Mission Control reconciliation, persisted queue ranks, and richer agent setup state.
...
- Validation: 35 focused tests passed; full Playwright suite passed 12 tests with one intentional boot-placeholder skip; the complete reconciled branch passed build. Repository-wide check on the current base remains blocked by two unrelated CLI errors fixed in PR 75.
