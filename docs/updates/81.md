...
- Group schedule routines and signal rules into one owner-facing routine without adding Dispatch persistence.
...
Tilde remains authoritative for schedules, signal rules, provider instances, and deliveries. Dispatch reconstructs each unified routine from metadata stamps on those resources.
...
- `@trytilde/dispatch-client-runtime` validates all client-consumed shapes and owns stale-while-revalidate polling and mutation reconciliation.
- `@trytilde/dispatch-ui` provides the agent details pane, routine editor, trigger builders, provider setup, and signals settings presentation for the DOM clients.
...
- `@trytilde/dispatch-control-service`: projects unified routine CRUD and test runs over Tilde ChatKit routines and signal rules; manages provider instances and deliveries; rolls back partial creates and refuses unsafe truncation of unpaginated signal lists.
- `@trytilde/dispatch-client-runtime`: adds routine and signal contracts, client calls, store slices, polling, cron helpers, and run-history composition.
- `@trytilde/dispatch-ui`: adds the resizable details pane, routine overview/editor, schedule and event trigger editors, provider connection dialog, and signal settings inventory.
- `@trytilde/dispatch-web`: adds details and signals routes, URL-addressable routine drill-in, settings navigation, and the shared UI wiring used by Electron.
