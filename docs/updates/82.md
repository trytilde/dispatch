...
- New Tilde API has atomic bulk add and remove for one tool provider instance. Dispatch must use it.
...
```mermaid
flowchart LR
    D[Dispatch deploy lifecycle] --> A[Agent Provider]
...
- Workspace tooling
  ...
  - Add a Changeset for the fixed Dispatch package group, generated API client, and SDK.
...
Forks that use Dispatch's Tilde Agent Provider should take this change so large control-plane toolkits reconcile with one atomic mapping request instead of many separate requests.

Fork action:

1. Update to this Dispatch revision after the companion Tilde API bulk endpoints are available.
