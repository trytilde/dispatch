...
Restore the Computer preview as a floating bottom-left workspace surface.
...
flowchart LR
  ...
  W --> F[Floating bottom-left preview]
  E --> F
...
- `@tryopenbot/ui`: positions the 240-320px preview 16px from the chat pane's bottom-left; mirrors composer spacing; preserves compact takeover behavior.
- Browser coverage: proves ordinary web and Electron floating placement, stable chat bounds, composer displacement, toggle persistence, responsive fallback, and fullscreen takeover.
- Changeset: records the owner-visible fixed-group patch.
- Validation: 58 UI tests passed; 2 web tests passed; focused web/Electron Playwright passed after rebasing onto upstream `main`; `pnpm check` passed with 36 existing warnings and no errors; `pnpm build` passed including package/CLI verification and Android and iOS Expo exports; full Playwright passed 13 tests with one existing boot-stage test skipped.
