...
- Refresh Dispatch from the companion Tilde API ownership/security contract.
...
```mermaid
flowchart LR
    T[Tilde API OpenAPI] --> G[Dispatch generated API client]
    G --> S[Generated SDK schema]
    S --> C[Dispatch and fork consumers]
    O[Ownership and federation policy] --> T
```

The API remains the contract owner. Dispatch copies the versioned OpenAPI document and regenerates its internal client and SDK schema. Runtime resource authorization, encryption, migrations, and MCP federation stay in Tilde API.
