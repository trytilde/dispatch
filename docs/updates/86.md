...
- Old way bad: one owner action often caused several dependent requests from Dispatch to Tilde.
...
- New companion Tilde API PR 157 owns these product transactions as aggregate endpoints and publishes the ChatKit workspace WebSocket protocol as generated AsyncAPI.
...
flowchart LR
    ...
    R --> B[ChatKit workspace bootstrap and snapshots]
    ...
    A --> G[Typed Dispatch agent bundle]
    ...
    WS[Team ChatKit workspace WebSocket] --> R
```

- One team-wide ChatKit workspace observer remains correct because each owner client needs activity for inactive chats. HTTP owns mutations and authoritative snapshots.
...
- No state import/export behavior changes. ChatKit workspace revision is delivery metadata, and aggregate responses compose existing resources.

## Summarized package changes

- `@trytilde/dispatch-client-runtime`
  - Add ChatKit workspace bootstrap, conversation snapshot, and submit-turn contracts.
...
- `@trytilde/dispatch-web` and Expo mobile
...
Forks should deploy companion Tilde API PR 157 before applying this Dispatch update. The new Dispatch runtime calls aggregate routes that older API deployments do not expose.

Fork action:

1. Merge and deploy Tilde API PR 157.
2. Update the Dispatch fork and install workspace dependencies normally.
...
5. Confirm ChatKit workspace loads, a turn sends, an attachment uploads, a connector binds, and Plugins loads.
