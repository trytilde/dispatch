...
- `tilde dev` previously sent owner-created agents to the trusted development sandbox, even though the active HMR runtime belonged to the operator's local checkout.
...
- The live tunnel origin is process-owned local state, so `tilde new-agent` reconciles the new agent against the active runtime without persisting a stale deployment origin.
...
flowchart LR
    ...
    C -->|tilde dev| L[Active local checkout]
