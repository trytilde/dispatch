...
- Deploy web, control, and isolated authored-agent functions as one atomic Dispatch runtime.
...
```mermaid
flowchart LR
    C[Dispatch configuration] --> R[One runtime provider]
    R --> W[Web assets]
...
Merge and deploy `trytilde/api#168` before adopting this update. Consolidated Vercel installations should set `VERCEL_RUNTIME_PROJECT`; the previous control project remains a compatibility read fallback. Deploy through `tilde deploy` so the combined runtime is built and health-checked, Tilde agent endpoints are cut over, `AGENT_SERVICE_ORIGIN` is persisted, and any legacy local agent service is retired in the safe order. Explicitly split installations may keep their existing provider composition.
