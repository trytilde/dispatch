...
- Make one Tilde Agent Resource Bundle the durable external owner of every authored Dispatch agent's runtime resources.
...
- Keep bundled skills, ChatKit tools, the default memory bank, and a deterministic avatar aligned with repository-authored agent state.
...
- `pnpm --filter @trytilde/dispatch-control-service test`: 63 passed.
- `pnpm --filter @trytilde/dispatch-agent-provider test`: 14 passed.
