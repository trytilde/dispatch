...
- `packages/git-provider`
  ...
  - Separate tenant DNS slug from opaque `org_...` JWT issuer; pass explicit SDK service origins.
...
3. Run interactive `openbot init`; select `exe.dev single VM`; provide existing VM name, sizing,
   checkout path, sync choice, GitHub owner/repo, tenant slug, opaque `org_...` identifier, and
   organization PKCS8 key in setup-only prompt.
