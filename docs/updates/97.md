...
- `/workspace/dispatch` symlinks live runtime checkout. Agent edits hit watched source directly.
...
- Explicit `--skip-agent-reconcile` and `DISPATCH_SKIP_AGENT_RECONCILE=1` recovery controls let an
...
flowchart LR
  ...
  VNC --> Host
  Host --> Workspace["/workspace/dispatch -> live checkout"]
...
decrypted Dispatch configuration. Untrusted third-party agents must not use this mode.
...
- Validation
  ...
  - Production proof on `our-dispatch.exe.xyz`: Code Storage fast-forward, systemd active with zero
  ...
  - OAuth proof: `/auth/login` emits `https://our-dispatch.exe.xyz/auth/callback`; Tilde accepts the
...
Dispatch hosting, fork-update, consolidated-runtime, and pricing tasks. Durable relevant evidence:
...
3. Run interactive `tilde init`; select `exe.dev single VM`; provide existing VM name, sizing,
