  ...
  return Promise.all(
    entries
      ...
      .map(async (entry) => {
        ...
        await access(path);
        await Promise.all(
          requiredComputerToolFiles.map((name) => access(resolve(agentDirectory, "tools", name))),
        );
    ...
    agents
      ...
      .map((agent) => {
        if (!/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(agent.slug))
          throw new Error(`Invalid agent directory: ${agent.slug}`);
        return agent;
      }),
  );
}
