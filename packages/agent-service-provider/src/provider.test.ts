...
describe("agent service artifacts", () => {
  it("rejects agents missing a required computer tool", async () => {
    ...
    await writeFile(
      join(root, "configuration/agents/incomplete/agent.ts"),
  ...
  it("discovers stable slugs and emits one independently bundled Vercel function per agent", async () => {
    ...
    for (const slug of ["alpha", "beta"]) {
      ...
      for (const name of [
        ...
        await writeFile(
          join(root, `configuration/agents/${slug}/tools/${name}`),
          "export default {}\n",
        );
      }
    }
    await writeFile(
      join(root, "configuration/agents/alpha/agent.ts"),
      "export default async function endpoint() { return new Response('alpha') }\n",
    );
    await writeFile(
      join(root, "configuration/agents/beta/agent.ts"),
