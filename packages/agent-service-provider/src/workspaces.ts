...
): Promise<readonly ComputerAgentWorkspace[]> {
...
async function filesBelow(root: string, directory: string): Promise<ComputerSeedFile[]> {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (entry.isSymbolicLink())
      throw new Error(`Agent workspace symlinks are not supported: ${path}`);
    if (entry.isDirectory()) files.push(...(await filesBelow(root, path)));
    else if (entry.isFile())
      files.push({ path: relative(root, path), content: new Uint8Array(await readFile(path)) });
  }
  return files;
}
