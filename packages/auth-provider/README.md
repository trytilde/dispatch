...
- `TildeAuthProvider` reconciles the installation's Tilde OIDC registration, registers the installation-specific public client, validates audience-restricted owner access tokens, and implements the `AuthProvider` contract. Development reconciliation includes the local Vite callback origins without replacing the deployed callback.
