...
export async function developmentSandboxSourceFiles(repositoryRoot: string): Promise<readonly ComputerSeedFile[]> {
  ...
  return Promise.all(paths.map(async (path) => {
    ...
    if (!metadata.isFile()) throw new Error(`Development sandbox source must be a regular file: ${path}`);
    return {
      path: `openbot/${path}`,
...
function isSafeDevelopmentSourcePath(path: string): boolean {
  if (!path || path.startsWith("/") || path.includes("\0") || path.split("/").some((part) => part === "..")) return false;
  if (path === ".env" || path.endsWith("/.env") || /(?:^|\/)\.env\.(?!example$)/.test(path)) return false;
  if (path === "configuration/secrets.yaml") return false;
  const parts = path.split("/");
  return !path.startsWith(".openbot-deploy/") && !parts.includes("node_modules") && !parts.includes("dist");
}
