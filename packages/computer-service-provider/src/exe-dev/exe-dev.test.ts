  ...
  it("routes noVNC and its WebSocket through the public owner origin", async () => {
    const computer = {
      ...
      previewAgentDesktop: vi.fn().mockResolvedValue({
        url: new URL(
          "http://127.0.0.1:6080/dispatch.html?path=websockify%3Ftoken%3Dcap&scale=true",
        ),
        expiresAt: new Date("2030-01-01T00:00:00Z"),
      }),
    } as unknown as ComputerProvider;
    const provider = new ExeDevComputerProvider({
      platform: new ExeDevPlatform({ vm: "dispatch" }),
      computer,
    });
    ...
    expect(endpoint.url.origin).toBe("https://dispatch.exe.xyz");
    expect(endpoint.url.pathname).toBe("/computer-vnc/dispatch.html");
