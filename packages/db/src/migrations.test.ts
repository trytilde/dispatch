...
test("migrates the prototype schema to control-plane-only state", async () => {
  ...
  await legacy.execute(
    "CREATE TABLE installations (id TEXT PRIMARY KEY NOT NULL, phase TEXT NOT NULL, onboarding_step TEXT NOT NULL, public_origin TEXT, secret_salt TEXT, wrapped_data_key TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)",
  );
  await legacy.execute(
    "CREATE TABLE agents (id TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, role TEXT NOT NULL, tools_json TEXT NOT NULL, created_at INTEGER NOT NULL)",
  );
  await legacy.execute({
    sql: "INSERT INTO installations VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    args: ["default", "onboarding", "jobs", "https://openbot.test", "obsolete", "obsolete", 1, 2],
  });
  legacy.close();
  ...
  expect(installation.rows[0]).toMatchObject({
    id: "default",
    phase: "onboarding",
