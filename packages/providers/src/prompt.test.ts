...
const request = {
  ...
  skills: [{ id: "skill-one", name: "web-investigation", description: "Research a question with cited sources.", version: 1 }],
};
...
describe("OpenBotPromptProvider", () => {
  it("assembles deterministic ordered sections and capability summaries", async () => {
    const plugin: PromptPlugin = {
      id: "installation",
      contribute: () => ({ id: "installation", priority: 500, cache: "session", content: "Installation rule." }),
    };
    ...
    expect(first.sections.map((section) => section.id)).toEqual([
      "identity", "work-policy", "tilde-runtime", "skill-catalog", "installation", "turn-context",
    ]);
  ...
  it("rejects duplicate plugin section ids", async () => {
    const provider = new OpenBotPromptProvider({
      plugins: [{ id: "duplicate", contribute: () => ({ id: "identity", priority: 1, cache: "stable", content: "bad" }) }],
    });
    await expect(provider.compose(request, { requestId: "one" })).rejects.toMatchObject({ code: "invalid_configuration" });
  });
  ...
  it("uses the currently selected provider's injected instructions", async () => {
    ...
    expect(local.sections.map((section) => section.id)).toContain("provider.sandbox-provider.local-box");
    expect(remote.sections.map((section) => section.id)).toContain("provider.sandbox-provider.remote-box");
