...
export class OpenBotPromptProvider implements PromptProvider {
  ...
  async compose(request: PromptRequest, context: ProviderCallContext): Promise<ComposedPrompt> {
    const contributed = await Promise.all(
      this.#plugins.map((plugin) => plugin.contribute(request, context)),
    );
    ...
    for (const section of sections) {
      if (ids.has(section.id))
        throw new ProviderError(
          ...
          `Duplicate prompt section id: ${section.id}`,
        );
      ids.add(section.id);
    }
    ...
    if (system.length > this.#maxPromptCharacters) {
      throw new ProviderError(
        "invalid_configuration",
        `Composed prompt exceeds ${this.#maxPromptCharacters} characters`,
      );
    }
    return {
      system,
...
export function defaultPromptPlugins(
  providers: OpenBotPromptProviderOptions["providers"] = {},
): readonly PromptPlugin[] {
  return [
    identityPlugin,
    workPolicyPlugin,
...
const tildeRuntimePlugin: PromptPlugin = {
  ...
  contribute(request) {
    ...
    if (!request.capabilities.runtimeMcp) {
      lines.push(
        "- The Tilde runtime MCP server is not configured for this turn. Do not claim that managed tools, skills, or memory were used.",
      );
    }
  ...
  contribute(request) {
    ...
    return section(
      ...
      [
        ...
        ...entries,
        ...(request.skills.length > entries.length
          ? [
              `- ${request.skills.length - entries.length} additional skills are available through search.`,
            ]
          : []),
      ].join("\n"),
    );
...
function normalizeSection(value: PromptSection, maxCharacters: number): PromptSection {
  ...
  if (!/^[a-z0-9][a-z0-9._-]*$/i.test(id))
    throw new ProviderError("invalid_configuration", `Invalid prompt section id: ${value.id}`);
  const content = value.content.replaceAll("\0", "").trim();
  if (content.length > maxCharacters)
    throw new ProviderError(
      "invalid_configuration",
      `Prompt section ${id} exceeds ${maxCharacters} characters`,
    );
