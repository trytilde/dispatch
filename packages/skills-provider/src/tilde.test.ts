...
describe("TildeSkillProvider", () => {
  ...
  it("uses the typed SDK for skill and registry control operations", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        if (request.method === "POST" && url.pathname.endsWith("/skill-registry"))
          return Response.json(registry());
        if (request.method === "POST" && url.pathname.endsWith("/skill"))
          return Response.json(skill());
        if (url.pathname.endsWith("/skill"))
          return Response.json({ items: [skill()], next_page_token: "next" });
    ...
    const created = await provider.createSkill(
      { name: "Research", description: "Research carefully", content: "# Research" },
      context,
    );
    const registered = await provider.registerSkills(
      { name: "OpenBot", description: "OpenBot skills", skillIds: [created.id] },
      context,
    );

    expect(listed).toMatchObject({
      items: [{ id: "skill-one", sourceKind: "repository" }],
      nextPageToken: "next",
    });
  ...
  it("verifies package assets before writing them to a destination", async () => {
    ...
    const manifest = await new TildeSkillProvider(config).materializeSkillAssets(
      "skill-one",
      {
    ...
    expect(writes).toEqual([
      { path: "scripts/run.sh", content: "asset content", executable: true },
    ]);
  });
  ...
  it("exposes only discovery and read operations as model tools", async () => {
    ...
    expect(
      provider.injectPromptPart({ agentId: "agent-one", sessionId: "session-one" }, context),
    ).toContain("search_skills");
    expect(
      await tools[0]?.execute?.(
        { query: "research", limit: 5 },
        { toolCallId: "call-one", messages: [] },
      ),
    ).toMatchObject([{ id: "skill-one" }]);
  });
});
