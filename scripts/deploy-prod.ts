...
const TILDE_PENDING_IMPORT_STATUSES = new Set(["queued", "validating", "applying", "rolling_back"]);
...
export function redact(value: string, secrets: Iterable<string>): string {
  ...
  return redacted.replace(
    /(TILDE_(?:API_KEY|WEBHOOK_SIGNING_KEY)|OPENAI_API_KEY|OPENBOT_SETUP_CODE|DATABASE_AUTH_TOKEN)=([^\s]+)/g,
    "$1=[REDACTED]",
  );
}
...
export function findMarketplaceResources(value: unknown): Array<Record<string, unknown>> {
  ...
  const visit = (candidate: unknown): void => {
    ...
    if (
      (serialized.includes("tursocloud") || serialized.includes('"provider":"turso cloud"')) &&
...
async function main(): Promise<void> {
  ...
  if (!options.yes && !options.dryRun)
    throw new Error("Production deployment requires --yes (or use --dry-run)");
  ...
  if (!process.env.TILDE_BEARER_TOKEN && !process.env.TILDE_API_KEY)
    missing.push("TILDE_BEARER_TOKEN" as (typeof required)[number]);
  if (missing.length)
    throw new Error(`Missing production deployment variables: ${[...new Set(missing)].join(", ")}`);
  ...
  await step(
    "validate",
    async () => {
      await run("vp", ["run", "check"], { env });
      await run("vp", ["run", "build"], { env });
    },
    false,
  );
  ...
  await step("project", async () => {
    ...
    const linked = JSON.parse(await readFile(resolve(".vercel/project.json"), "utf8")) as {
      projectId: string;
      orgId: string;
      projectName?: string;
    };
    state.project = {
      id: linked.projectId,
      name: linked.projectName || projectName,
      teamId: linked.orgId,
    };
  });
  ...
  await step("turso", async () => {
    const listed = await vercel(
      ["integration", "list", "--integration", TURSO_INTEGRATION, "--json"],
      env,
    );
    ...
    if (!resource) {
      const added = await vercel(
        [
          ...
          `${projectName}-db`,
          "--metadata",
          `region=${process.env.TURSO_PRIMARY_REGION || "iad1"}`,
          "--plan",
          process.env.TURSO_PLAN || "starter",
          "--environment",
          "production",
          "--no-env-pull",
          "--json",
      ...
      resource =
        findMarketplaceResources(parseJsonOutput(added.stdout))[0] ??
        (parseJsonOutput(added.stdout) as Record<string, unknown>);
    }
    state.turso = {
    ...
    try {
      ...
      if (!databaseUrl || !databaseToken)
        throw new Error("Turso did not expose TURSO_DATABASE_URL and TURSO_AUTH_TOKEN");
  ...
  await step("tilde_import", async () => {
    ...
    for (let attempt = 0; attempt < 3 && !imported; attempt += 1) {
      try {
        ...
        if (
          message.includes("resource chatkit/agent/openbot-gateway conflicts with existing state")
        ) {
        ...
        if (
          message.includes("resource chatkit/provider/openbot-web conflicts with existing state")
        ) {
    ...
    if (!imported.environment?.TILDE_API_KEY && !process.env.TILDE_WEBHOOK_SIGNING_KEY) {
    ...
    const apiKey = imported.environment?.TILDE_API_KEY ?? process.env.OPENBOT_TILDE_API_KEY;
    const webhookSigningKey =
      imported.environment?.TILDE_WEBHOOK_SIGNING_KEY ??
      process.env.OPENBOT_TILDE_WEBHOOK_SIGNING_KEY;
    if (!apiKey || !webhookSigningKey)
      throw new Error("Tilde import did not return TILDE_API_KEY and TILDE_WEBHOOK_SIGNING_KEY");
    secrets.add(apiKey);
    ...
    await setVercelEnvironment("OPENBOT_TILDE_WEBHOOK_SIGNING_KEY", webhookSigningKey, env);
    ...
    await setVercelEnvironment("OPENBOT_TILDE_AGENT_ID", state.tilde.agentId, env);
    await setVercelEnvironment("OPENBOT_TILDE_UI_PROVIDER_ID", state.tilde.providerId, env);
  });

  await step("configure", async () => {
    if (!state.publicOrigin) throw new Error("Deployment origin is missing");
    let tildeApiKey =
      process.env.OPENBOT_IMPORTED_TILDE_API_KEY ?? process.env.OPENBOT_TILDE_API_KEY;
    let signingKey =
    ...
    await configureInstallation(state.publicOrigin, setupCode, {
      ...
      tildeRuntimeMcpServerId:
        state.tilde?.runtimeMcpServerId ?? process.env.TILDE_RUNTIME_MCP_SERVER_ID ?? "",
      tildeSkillRegistryId:
        state.tilde?.skillRegistryId ?? process.env.TILDE_SKILL_REGISTRY_ID ?? "",
  ...
  await step("reconcile", async () => {
    ...
    const response = await fetch(`${state.publicOrigin}/api/admin/reconcile`, {
      method: "POST",
      headers: { cookie },
    });
    if (!response.ok)
      throw new Error(
        `Repository reconciliation failed (${response.status}): ${await response.text()}`,
      );
  });
...
export function renderTildeState(source: string, endpointUrl: string): string {
  ...
  if (!document || typeof document !== "object")
    throw new Error("Tilde state must be a YAML object");
...
export function parseTildeImportSummary(value: unknown): TildeImportResult {
  ...
  if (!importId || !status)
    throw new Error("Tilde state import response is missing its ID or status");
  ...
  if (status !== "applied")
    throw new Error(`Tilde state import stopped in unexpected status: ${status}`);

  const outputs =
    summary.outputs && typeof summary.outputs === "object" && !Array.isArray(summary.outputs)
  ...
  if (!Object.keys(resources).length)
    throw new Error("Tilde state import completed without resource outputs");

  const environmentFile = outputs.environment_file;
  const contents =
    environmentFile && typeof environmentFile === "object" && !Array.isArray(environmentFile)
...
async function importTildeState(input: {
  ...
  if (!importId || !initialStatus)
    throw new Error("Tilde state import did not return an import ID or status");
...
async function findExactTildeProviderId(input: {
  ...
  const items =
    response && typeof response === "object" && !Array.isArray(response)
      ? (response as Record<string, unknown>).items
      : undefined;
  const matches = Array.isArray(items) ? items.filter(isExactTildeProvider) : [];
  if (matches.length !== 1) {
    throw new Error(
...
export async function loadSopsEnvironment(
  ...
  if (result.code !== 0) {
    throw new Error(
      `Unable to decrypt ${path} with SOPS (${result.code}): ${result.stderr.trim() || "unknown error"}`,
    );
  }
...
async function loadSetupCode(options: DeployOptions): Promise<string> {
  ...
  if (fromEnvironment) {
    if (fromEnvironment.length < 16)
      throw new Error("OPENBOT_SETUP_CODE must contain at least 16 characters");
...
async function vercel(
  ...
  return run(
    "vp",
    ["exec", "vercel", ...args, "--scope", env.VERCEL_TEAM_ID!, "--non-interactive"],
    { env, allowFailure },
  );
}
...
async function pullVercelProductionEnvironment(
  ...
  try {
    ...
    await vercel(["env", "pull", path, "--environment", "production", "--yes"], env);
...
async function deployVercel(env: NodeJS.ProcessEnv): Promise<string> {
  ...
  if (!urls?.length)
    throw new Error(`Vercel deploy did not return a URL: ${result.stdout || result.stderr}`);
  return urls.at(-1)!;
}
...
async function productionOrigin(
  ...
  const inspected = await vercel(
    ["inspect", deploymentUrl, "--wait", "--timeout", "10m", "--json"],
    env,
  );
...
async function unlock(origin: string, setupCode: string): Promise<string> {
  ...
  if (!response.ok)
    throw new Error(`Setup unlock failed (${response.status}): ${await response.text()}`);
...
async function connectJson(
  ...
  if (!response.ok)
    throw new Error(`${service}.${method} failed (${response.status}): ${await response.text()}`);
...
async function provisionSnapshot(credentials: {
  token: string;
  projectId: string;
  teamId: string;
}): Promise<string> {
  ...
  const sandbox = await Sandbox.create({
    ...credentials,
    image: process.env.OPENBOT_VERCEL_SANDBOX_IMAGE || "vercel/sandbox/universal:latest",
    ports: [6080, 4101],
    timeout: 30 * 60 * 1000,
  });
...
async function productionSmoke(origin: string, setupCode: string): Promise<void> {
      ...
      messages: [
        {
          id: "deploy-smoke",
          role: "user",
          parts: [{ type: "text", text: "Reply with exactly: openbot-ok" }],
        },
      ],
    }),
  ...
  if (!chat.ok)
    throw new Error(`Tilde ChatKit smoke failed (${chat.status}): ${await chat.text()}`);
  const chatBody = (await chat.json()) as { sessionId?: unknown; messages?: unknown[] };
  if (typeof chatBody.sessionId !== "string" || !chatBody.messages?.length)
    throw new Error("Tilde ChatKit smoke returned no session or messages");
  ...
  try {
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      ...
      if ((action.exitCode ?? 0) === 0 && String(action.stdout ?? "").includes("openbot-cua-ok"))
        break;
      await new Promise((resolvePromise) => setTimeout(resolvePromise, 1_000));
    }
    if (
      !action ||
      (action.exitCode ?? 0) !== 0 ||
      !String(action.stdout ?? "").includes("openbot-cua-ok")
    ) {
    ...
    if (!noVnc.ok || !(await noVnc.text()).toLowerCase().includes("novnc"))
      throw new Error(`noVNC smoke failed (${noVnc.status})`);
  } finally {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  main().catch((error: unknown) => {
    const knownSecrets = Object.entries(process.env)
      .filter(
        ([name, value]) => Boolean(value) && /(?:TOKEN|SECRET|KEY|SETUP_CODE|PASSWORD)/.test(name),
      )
      .map(([, value]) => value!);
    process.stderr.write(
      `${redact(error instanceof Error ? error.message : String(error), knownSecrets)}\n`,
    );
    process.exitCode = 1;
  });
}
