...
await runChecked("vp", ["run", "contracts:generate"], env);
await runChecked("vp", ["run", "--filter", "@openbot/box-host", "build"], env);
await migrate();
...
const server = run("vp", ["run", "--filter", "@openbot/server", "dev"], env);
const web = run("vp", ["run", "--filter", "@openbot/web", "dev", "--port", webPort], env);
...
if (canLaunchDesktop) {
  ...
  children.push(run("vp", ["run", "--filter", "@openbot/desktop", "dev"], desktopEnv));
} else {
  console.log(
