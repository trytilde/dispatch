...
async function setup(rest: readonly string[]): Promise<void> {
  ...
  await withProgress("Preparing repository directories", () =>
    Promise.all(directories.map((directory) => mkdir(resolve(directory), { recursive: true }))),
  );
  const result = {
    directories,
    next: "Configure .env, then run `vp run openbot doctor` and `vp run openbot dev`.",
  };
  if (wantsJson(rest)) return printJson(result);
  show(
    <Success title="Repository directories are ready">
      <TextLines lines={[...directories, result.next]} />
    </Success>,
  );
}
...
async function doctor(rest: readonly string[]): Promise<void> {
  const [repository, statuses] = await withProgress("Checking configuration and providers", () =>
    Promise.all([loadRepository(), providerStatuses()]),
  );
...
async function providers(rest: string[]): Promise<void> {
  ...
  if ((args[0] ?? "list") !== "list") throw new Error("Use `vp run openbot providers list`");
...
async function sync(rest: string[]): Promise<void> {
  ...
  if (prune && !rest.includes("--yes"))
    throw new Error("--prune disables removed remote agents and requires --yes");
  const report = await withProgress("Synchronizing agents and skills", () =>
    reconcileRepository({ prune }),
  );
...
async function status(rest: readonly string[]): Promise<void> {
  ...
  const result = await withProgress("Loading repository registrations", async () => {
    const [agents, skills] = await Promise.all([
      db.select().from(agentRegistrations),
      db.select().from(skillRegistrations),
    ]);
...
function repositorySummary(
  ...
  return {
    ...
    agents: repository.agents.length,
    skills: repository.skills.length,
    providers: repository.providerPlugins.length,
  };
}
...
async function delegate(script: string, args: readonly string[]): Promise<void> {
  if (process.stdout.isTTY) show(<Success title={`Starting vp run ${script}`} />);
  const child = spawn("vp", ["run", script, ...args], {
    stdio: "inherit",
